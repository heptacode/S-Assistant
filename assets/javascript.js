...
/*
    GLOBAL VARIABLES
*/
var server = "https://assistant.hyunwoo.dev/",
  cdnServer = "https://assistant-cdn.hyunwoo.dev/",
  modifyCode,
  timer_boardTable,
  timer_consoleTable,
...
$(function () {
  ...
  $(".board").click(function () {
    ...
    $(".board").html("읽어들이는 중").addClass("board-active");
  });
  $(".btn-board-close").click(function () {
    $(this).fadeOut();
    clearInterval(timer_boardTable);
    $(".board-wrap").removeClass("board-wrap-active");
    $(".board").html('<div class="board-default"><div class="board-icon"></div><div>보드</div></div>').removeClass("board-active");
  ...
  $("input:checkbox").click(function () {
    $("#create-checkbox-postNow").is(":checked") ? $(".form-row-create-post").slideUp() : $(".form-row-create-post").slideDown().css("display", "flex");
    $("#create-checkbox-unlimited").is(":checked") ? $(".form-row-create-deadline").slideUp() : $(".form-row-create-deadline").slideDown().css("display", "flex");
    $("#modify-checkbox-postNow").is(":checked") ? $(".form-row-modify-post").slideUp() : $(".form-row-modify-post").slideDown().css("display", "flex");
    $("#modify-checkbox-unlimited").is(":checked") ? $(".form-row-modify-deadline").slideUp() : $(".form-row-modify-deadline").slideDown().css("display", "flex");
  });
  ...
  $(".code-reissue").click(function () {
    ...
    codeSet();
    setTimeout(function () {
      $(".code-reissue").removeClass("code-reissue-disabled").prop("disabled", false);
    }, 1000);
  });
        ...
        function (response) {
          response
            ? ($("#row_" + modifyCode).addClass("trRemove"),
              ...
              $(".modal-modify").removeClass("active"),
              $(".modal-modify-content").addClass("slideDown").removeClass("slideUp"),
              $(".form-remove").removeClass("form-remove-disabled").prop("disabled", false),
              (window.onbeforeunload = true))
            : ($(".form-remove").removeClass("form-remove-disabled").prop("disabled", false), alert("폼 삭제에 실패하였습니다.\n일시적인 네트워크 오류일 수 있으므로 잠시 후에 다시 시도하십시오."));
        }
      );
    }
  });
...
function signIn() {
  ...
  if (valid) {
    ...
    $.post(
      ...
      function (response) {
        response
          ? location.replace(server + $("#input-code").val() + "#" + $("#input-name").val())
          : ($(".btn-signIn").css("display", "block"), $(".load-signIn").css("display", "none"), $(".notice-inputError").fadeIn());
      }
    );
  }
}
...
function boardTable() {
  $.post("proxy.php", { do: "boardTable" }, function (response) {
    response
      ? ($(".btn-board-close").css("display", "block"),
        $(".board").html("<h2>S-Board</h2><table><thead><tr><th>레이블</th><th>제출</th><th>마감</th><th>게시자</th></tr></thead><tbody>" + response + "</tbody></table>"))
      : ($(".btn-board-close").css("display", "block"), $(".board").html("<h3>현재 표시 가능한 폼 없음</h3>"));
  });
}
...
function consoleTable() {
  ...
  $.post("proxy.php", { do: "consoleTable" }, function (response) {
    response
      ? $(".console-table").html(
          '<table><thead><tr><th>레이블</th><th>코드</th><th>제출</th><th class="td-auto">생성</th><th class="td-auto">게시</th><th class="td-auto">마감</th><th class="td-auto">마감 후 제출</th><th class="td-auto">FB 인증</th><th class="td-auto">권한</th></tr></thead><tbody>' +
            response +
            "</tbody></table>"
        )
      : $(".console-table").html('<div class="notice-nodata">+ 버튼을 눌러 새로운 폼을 생성하세요!</div>');
  });
}
  ...
  if (location.pathname != "/") {
    ...
    if (!!Cookies.get("fbValid")) {
      ...
      $(".pf-icon")
        ...
        .css("display", "block");
      $(".pf-name").text(Cookies.get("fbName")).css("display", "block");
      ...
      if (!location.hash || location.hash == "#") {
        ...
        $(".pf-name").css("display", "none").text("로드 중");
        $(".submit-subtitle").text(Cookies.get("fbName"));
    ...
    $(".field").css("overflow-y", "hidden").css("display", "block");
